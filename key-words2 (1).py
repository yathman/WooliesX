...
def connect_to_azure(data):
    azure_url = "https://pioneeranalytics.cognitiveservices.azure.com/"
    language_api_url = "{}text/analytics/v2.1/languages".format(azure_url)
    key_phrases_url = "{}text/analytics/v3.0/keyPhrases".format(azure_url)
    subscription_key = data["azure"]["subscription_key"]
    return language_api_url, key_phrases_url, subscription_key
...
def key_phrase_identifier(headers, key_phrases_url, document_format):
    response = requests.post(
        key_phrases_url, headers=headers, json=document_format)
    return response.json()
...
def main():
    ...
    language_api_url, key_phrases_url, subscription_key = connect_to_azure(data)
    ...
    key_phrases = key_phrase_identifier(headers, key_phrases_url, document_format)
